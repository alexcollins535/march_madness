...
#=====================================================
# CURRENTLY: Re-evaluating features after data correction
#=====================================================
...
#   7: TARGET FT% - GBR - RMSE: 12.3526
#   8: TARGET 2PT% - GBR - RMSE: 0.0875
...
def add_opponent_seed(combined):
    # Build a lookup: (Team, YEAR) -> Seed
    seed_lookup = (
        combined[['Team', 'YEAR', 'Seed']]
        .drop_duplicates()
        .set_index(['Team', 'YEAR'])['Seed']
    )

    # Sanity check - flag any teams with multiple seeds in the same year
    duplicates = (
        combined[['Team', 'YEAR', 'Seed']]
        .drop_duplicates()
        .groupby(['Team', 'YEAR'])
    ...
    if (duplicates > 1).any():
        print('[WARN] Some teams have multiple seeds in the same year:')
        print(duplicates[duplicates > 1])

    # Map opponent seed using Opp and YEAR
    combined['Opp Seed'] = combined.set_index(['OPP', 'YEAR']).index.map(seed_lookup)

    # Check for any failed lookups
    n_missing = combined['Opp Seed'].isna().sum()
    if n_missing > 0:
        print(f'[WARN] {n_missing} rows could not be mapped to an opponent seed.')
        print(combined[combined['Opp Seed'].isna()][['Team', 'OPP', 'YEAR']].head(10))
...
def get_model_feature_list(target_variable, combined_df):
    ...
    if target_variable == 'TARGET POSS':
        feature_list.extend(['T1_TEMPO', 'T2_TEMPO', 'T1_FGA', 'T2_FGA', 'T1_FTA', 'T2_FTA', 'T1_OPPG', 'T2_OPPG', 'T1_TOF', 'T2_TOF', 'T1_RPG', 'T2_RPG', 'T1_3PPG', 'T2_3PPG'])

    elif target_variable == 'TARGET FGA':
        feature_list.append('PRED POSS')

        feature_list.extend(['Seed', 'T1_APG', 'T1_FGA', 'T2_TEMPO', 'T1_ORPG', 'T2_DRPG', 'T2_OPPG', 'T2_FPG', 'T1_TPG', 'T2_ORPG', 'T1_FTA', 'T2_TM', 'T2_RM', 'T2_PPG'])

    elif target_variable == 'TARGET FG%':
        feature_list.extend(['PRED POSS', 'PRED FGA'])

        feature_list.extend(['Seed', 'Opp Seed', 'T1_FG%', 'T2_3PD%', 'T2_BPG', 'T1_FB', 'T1_Bench', 'T2_FTPG', 'T1_SM', 'T2_3PT%', 'T1_3PPG', 'T2_ORPG', 'T2_2PT FGM', 'T1_A:T', 'T1_3PTA', 'T2_SPG', 'T2_3PTA'])

    elif target_variable == 'TARGET 3PTA':
        feature_list.extend(['PRED FGA'])

        feature_list.extend(['T1_3PTA', 'T2_3PD%', 'T1_3PPG', 'T2_TPG', 'T2_APG', 'T1_ORPG', 'T2_OPPG', 'T2_PPG', 'T2_FTA', 'T2_TEMPO', 'T2_SPG', 'T2_FTPG', 'T2_DFG%', 'T2_SM', 'T2_DRPG', 'T1_A:T', 'T1_2PT FGM', 'T1_FGA', 'T2_TOF', 'T2_FG%', 'Seed', 'T1_FB', 'T1_DRPG'])
        
    elif target_variable == 'TARGET 3PT%':
        feature_list.extend(['Seed', 'T1_3PT%', 'T2_3PD%', 'T2_RM', 'T1_FT%', 'T1_A:T', 'T2_BPG', 'T1_EFG%', 'T1_ORPG', 'T1_TEMPO', 'T1_FG%', 'T1_RPG', 'T2_FT%', 'T1_Bench', 'T1_FGA', 'T1_2PT FGM', 'T2_DFG%', 'T1_FTA', 'T2_3PT%', 'T2_TEMPO', 'T1_DRPG', 'T1_OPPG', 'T2_SM', 'T2_DRPG', 'T1_3PPG', 'T1_SM', 'T1_TPG'])

    elif target_variable == 'TARGET FTA':
        feature_list.extend(['PRED POSS', 'PRED FGA'])

        feature_list.extend(['Seed', 'T1_FTA', 'T1_FTPG', 'T2_FPG', 'T2_APG', 'T1_ORPG', 'T1_FPG', 'T2_3PD%', 'T2_3PT%', 'T1_OPPG', 'T2_FTA', 'Opp Seed', 'T2_FG%'])
        
    elif target_variable == 'TARGET FT%':
        feature_list.extend(['PRED FGA'])

        feature_list.extend(['Seed', 'T1_FT%', 'T1_Bench', 'T1_APG', 'T1_2PT FGM', 'T2_3PT%', 'T2_SM', 'T2_PPG'])

    elif target_variable == 'TARGET 2PT%':
        feature_list.extend(['PRED POSS', 'PRED FGA'])

        feature_list.extend(['Seed', 'Opp Seed', 'T1_2PT FGM', 'T1_3PPG', 'T1_3PTA', 'T2_DFG%', 'T1_EFG%', 'T2_RM', 'T2_TPG', 'T1_3PT%', 'T1_FTPG', 'T2_TEMPO', 'T1_FB', 'T2_FB'])
...
def evaluate_features_to_add(combined, target_variable):
    ...
    if 'Seed' not in base_feature_list:
        candidate_features.append('Seed')
    if 'Opp Seed' not in base_feature_list:
        candidate_features.append('Opp Seed')
